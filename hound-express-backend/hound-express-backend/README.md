...
```sh
git clone https://github.com/nickjj/docker-django-example hound-express
cd hound-express
...
The app is named `hound-express` right now but chances are your app will be a different
name. Since the app is already created we'll need to do a find / replace on a
few variants of the string "hound-express" and update a few Docker related resources.
